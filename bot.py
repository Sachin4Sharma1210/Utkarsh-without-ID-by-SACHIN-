...
def _extract_sync(mobile, password, batch_id):
    ...
    try:
        ...
        if not jwt: return [], "❌ Login failed."
        ...
        if not user_id: return [], "❌ User ID not found."
        
        HEADERS["userid"] = user_id
        suffix = (user_id + "1524567456436545")[:16]
        key = "".join(key_chars[int(i)] for i in suffix).encode()
        iv = "".join(iv_chars[int(i)] for i in suffix).encode()
        ...
        dr3 = decrypt_and_load_json(u4.get("response"))
        data = dr3.get("data", [])
        ...
        for i in data:
            try:
                ...
                # Fixed the backslash syntax error here
                clean_title = tn.replace('/','_').replace(':','_').replace('|','_').replace('\\','_')
                fn = f"{fi}_{clean_title}.txt"
                ...
                with open(fn, "w", encoding="utf-8") as f:
                    ...
                    u5 = session.post(tiles_data_url, headers=h, data={'tile_input': de2, 'csrf_name': csrf_token}).json()
                    dr4 = decrypt_and_load_json(u5.get("response"))
                    ...
                    for subj in dr4.get("data", {}).get("list", []):
                        ...
                        u6 = session.post(layer_two_data_url, headers=h, data={'layer_two_input_data': de3, 'csrf_name': csrf_token}).json()
                        dr5 = decrypt_and_load_json(u6.get("response"))
                        ...
                        for topic in dr5.get("data", {}).get("list", []):
                            ...
                            u7 = session.post(layer_two_data_url, headers=h, data={'layer_two_input_data': de4, 'csrf_name': csrf_token}).json()
                            dr6 = decrypt_and_load_json(u7.get("response"))
                            
                            for video in dr6.get("data", {}).get("list", []):
                                jt = video.get("title", "Video")
                                jti = video.get("payload", {}).get("tile_id")
                                if not jti: continue
                                meta_payload = {"course_id": fi, "device_id": "x", "device_name": "x", "download_click": "0", "name": f"{video.get('id', '')}_0_0", "tile_id": jti, "type": "video"}
                                j5 = post_request(meta_source_url, meta_payload, key=key, iv=iv)
                                cj = j5.get("data", {})
                                vu = ""
                                if isinstance(cj, dict):
                                    qo = cj.get("bitrate_urls", [])
                                    if qo: vu = qo[-1].get("url") or qo[0].get("url")
                                    if not vu: vu = cj.get("link", "")
                                
                                if vu:
                                    pu = vu.split("?Expires=")[0] if "?Expires=" in vu else vu
                                    if not pu.startswith("http"): pu = f"https://www.youtube.com/embed/{pu}"
                                    f.write(f"{jt}: {pu}\n")
                files.append(fn)
                time.sleep(0.1)
            except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Bot is Online!\n\nSend: `mobile*password`", parse_mode=None)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if '*' in text:
        parts = text.split('*')
        if len(parts) == 2:
            user_sessions[uid] = {"mobile": parts[0], "password": parts[1]}
            await update.message.reply_text("✅ Credentials saved. Now send **Batch ID**.")
        return

    if uid not in user_sessions:
        await update.message.reply_text("🔒 First send `mobile*password`")
        return

    if text.isdigit():
        await update.message.reply_text("⏳ Extracting links... please wait.")
        creds = user_sessions[uid]
        files, err = await extract_courses(creds["mobile"], creds["password"], text)
...
def main():
    ...
    TOKEN = "8441306868:AAFiY_FTmyljnldJq6da8NcESkH5hVXCiLA"
